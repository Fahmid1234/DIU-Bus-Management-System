...
app.config['MAIL_USERNAME'] = 'mdfahmidbinmostafa@gmail.com'
app.config['MAIL_PASSWORD'] = 'glffppadhqigbvei'
    ...
    if request.method == "POST":
        ...
        if user is not None and len(user) > 0:
            ...
            if '@diu.edu.bd' in email:
                ...
                mydb.commit()
                message = Message(
                    'OTP', sender='mdfahmidbinmostafa@gmail.com', recipients=[email])
...
def pay_success():
    ...
    print(current_date)
    insert_query = "INSERT INTO payment_details (from_location, to_location, price, user_id, Date) VALUES (%s, " "%s, %s, %s ,%s) "
